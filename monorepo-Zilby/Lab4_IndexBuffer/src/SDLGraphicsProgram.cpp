...
// TODO: Add data to the indices
  ...
  glVertexAttribPointer(  1,   // Attribute 0, which will match layout in shader
                      ...
                      // If we have normals, then we need to jump 3*sizeof(GL_FLOAT) bytes to get
                      // to our next vertex.
                        (void*)(3*sizeof(GL_FLOAT))       // Pointer to the starting point of our data. If we are just grabbing vertices, this is 0. But if we have some other attribute, (stored in the same data structure), this may vary if the very first element is some different attribute.
                        // If we had some data after (say normals), then we would have an offset of 3*sizeof(GL_FLOAT) for example
  );
