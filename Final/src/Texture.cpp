...
void Texture::LoadPNGTexture(const std::string filepath){
    // Set member variable
    m_filepath = filepath;
    ...
    if(image == nullptr) 
    {
        ...
        throw(std::string("Failed to load png image: " + filepath));
    }
    ...
    glTexImage2D(GL_TEXTURE_2D,
                        ...
                        GL_RGBA,
                        GL_UNSIGNED_BYTE,
                        image); // Here is the raw pixel data
